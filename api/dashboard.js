// TradeMind — Vollständiges Dashboard als Serverless Function
// Tab 1: Real Portfolio (Preise + Stops bearbeiten + Trade eintragen)
// Tab 2: Paper Trades (Fund-Stats + Positionen + aufklappbare Strategien)
...
const STRATEGIES = [
  {
    id:'PS1', color:'#3498db', name:'Iran/Öl-Geopolitik', status:'🟢',
    tickers:['OXY','TTE.PA','NOVO-B.CO'],
    desc:`Solange der Iran-Konflikt die Straße von Hormuz bedroht, bleibt Öl teuer → Ölproduzenten profitieren.\n\n<strong>Kernthese:</strong> Iran + Houthi-Angriffe = Versicherungsprämie auf Öl. Jede Eskalation = bullisches Signal.\n<strong>Ausstieg wenn:</strong> Iran-Deal, Normalisierung der Tankerrouten, WTI dauerhaft unter 70$.`,
    conviction:'Hoch'
  },
  {
    id:'PS2', color:'#e67e22', name:'Tanker-Lag-These', status:'🟢',
    tickers:['FRO','DHT'],
    desc:`Wenn Öl steigt, folgen Tanker-Aktien mit 2-4 Wochen Verzögerung. Wir kaufen Tanker BEVOR sie nachziehen.\n\n<strong>Kernthese:</strong> Steigende Ölpreise = mehr Tanker-Nachfrage + höhere Frachtraten → Tanker-Aktien hinken nach.\n<strong>Ausstieg wenn:</strong> Öl fällt unter 70$ oder Frachtraten kollabieren.`,
    conviction:'Mittel'
  },
  {
    id:'PS3', color:'#2ecc71', name:'NATO/EU-Rüstung', status:'🟡',
    tickers:['ASML.AS','HO.PA'],
    desc:`Europa erhöht Verteidigungsbudgets massiv → Rüstungs- und Dual-Use-Firmen bekommen Aufträge.\n\n<strong>Kernthese:</strong> 2% BIP-Ziel NATO → DE + FR + UK erhöhen Ausgaben. ASML (Chips für Guidance), Thales (Elektronik, Radar).\n<strong>Risiko:</strong> Ukraine-Waffenstillstand könnte Euphorie ausbremsen.`,
    conviction:'Mittel'
  },
  {
    id:'PS4', color:'#f1c40f', name:'Edelmetalle/Miner', status:'🟡',
    tickers:['HL','PAAS'],
    desc:`Bei hoher Unsicherheit (VIX hoch, Geopolitik) fliehen Anleger in Gold/Silber → Minenproduzenten profitieren überproportional.\n\n<strong>Kernthese:</strong> Miner = gehebelte Wette auf Metallpreise. Bei Gold +20% → Miner oft +40-60%.\n<strong>Ausstieg wenn:</strong> VIX unter 18, Gold unter 2.500$.`,
    conviction:'Mittel'
  },
  {
    id:'PS5', color:'#8b4513', name:'Dünger/Agrar-Superzyklus', status:'🟡',
    tickers:['MOS','GLEN.L'],
    desc:`Russische Kali-Sanktionen + steigende Lebensmittelnachfrage → westliche Düngerproduzenten profitieren.\n\n<strong>Kernthese:</strong> Belarus + Russland = 40% des globalen Kali. Sanktionen = struktureller Engpass.\n<strong>Risiko:</strong> Sanktionslockerung, schlechte Ernte = weniger Nachfrage.`,
    conviction:'Niedrig'
  },
];
...
nav button{background:none;border:none;color:var(--muted);padding:12px 24px;font-size:14px;cursor:pointer;border-bottom:3px solid transparent;font-weight:500;flex:1;transition:color .2s}
...
.section-title{font-size:13px;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid var(--border)}
/* TABLE */
...
.stop-bar{height:3px;border-radius:2px;background:var(--border);margin-top:4px;width:100%;max-width:80px}
...
/* FORM */
.edit-grid{display:grid;grid-template-columns:1fr 1fr 1fr 1fr auto;gap:8px;align-items:center;padding:8px 0;border-bottom:1px solid var(--border)}
.edit-grid:last-child{border-bottom:none}
...
.strat-chevron{font-size:12px;color:var(--muted);transition:transform .2s}
.strat-chevron.open{transform:rotate(180deg)}
.strat-body{display:none;padding:0 16px 16px 66px;font-size:13px;line-height:1.7;color:var(--text)}
.strat-body.open{display:block}
.strat-tickers{display:flex;gap:6px;margin-top:10px;flex-wrap:wrap}
...
<header>
  ...
  <h1>TradeMind</h1>
  <button class="btn btn-muted" onclick="loadAll()" style="padding:6px 14px;font-size:12px">🔄</button>
...
<div id="tab-real" class="tab active">
  ...
  <div class="section">
    <div class="section-title">Stops &amp; Entries bearbeiten</div>
    <div id="edit-table"><div class="loading">Lädt…</div></div>
  </div>

  <div class="section">
    <div class="section-title">Trade eintragen</div>
...
<div id="tab-paper" class="tab">
  ...
  <div class="section">
    <div class="section-title">Strategien (zum Aufklappen)</div>
...
function pct(v,b){return b!=null&&b!==0?((v-b)/b*100):null}
function fmt(v,d=2){return v!=null?v.toFixed(d):null}
function pctHtml(v){
...
function stopCell(price, stop) {
  ...
  return \`<div><span style="color:\${col};font-weight:600">\${stop.toFixed(2)}€</span> <span class="muted" style="font-size:11px">(\${dStr} weg)</span></div>
          <div class="stop-bar"><div class="stop-fill" style="width:\${bar}%;background:\${col}"></div></div>\`;
}

function getPrice(px, ticker) {
...
function renderMacro(p) {
  ...
  const nkc=nk<-3?'var(--red)':nk<0?'var(--orange)':'var(--green)';
  document.getElementById('macro-strip').innerHTML=\`
    ...
    <div class="macro-item"><div class="macro-key">Nikkei</div><div class="macro-val" style="color:\${nkc}">\${nk>=0?'+':''}\${nk.toFixed(1)}%</div><div class="macro-sub muted">225</div></div>
...
function renderReal(cfg, prices) {
  ...
  const rows=pos.map(p=>{
    const pr=getPrice(px,p.ticker),eur=pr?.eur??null,chg=pr?.dayChange??null;
    const pnl=pct(eur,p.entry_eur);
    const priceStr=eur!=null?\`<strong>\${eur.toFixed(2)}€</strong>\`:'<span class="muted">—</span>';
    return \`<tr>
      <td><div class="ticker-name">\${p.ticker}</div><div class="ticker-sub">\${p.name||''}</div></td>
      <td class="muted">\${fmt(p.entry_eur)??'—'}€</td>
      <td>\${priceStr}</td>
      <td>\${pctHtml(chg)}</td>
      <td>\${pctHtml(pnl)}</td>
...
function renderEdit(cfg) {
  const pos=(cfg.positions||[]).filter(p=>p.status!=='CLOSED');
  if(!pos.length){document.getElementById('edit-table').innerHTML='<p class="muted">Keine Positionen</p>';return;}
  const rows=pos.map(p=>\`
    <div class="edit-grid">
      ...
      <div style="padding-top:16px"><button class="btn btn-primary btn-sm" onclick="savePos('\${p.ticker}')">💾</button><span id="e-status-\${p.ticker}" class="save-msg"></span></div>
    </div>\`).join('');
  document.getElementById('edit-table').innerHTML=\`<div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:12px 16px">\${rows}</div>\`;
}

function renderPaper(prices) {
  const px=prices.prices||{};
  let totalPnlPct=0,wins=0,count=0,totalVal=0;
  const START=1000, CASH=71;
  const posValue=900; // ~Kapital minus Bares

  const rows=PAPER.map(p=>{
    const pr=getPrice(px,p.ticker),eur=pr?.eur??null,chg=pr?.dayChange??null;
    const pnl=pct(eur,p.entry);
    if(pnl!=null){count++;totalPnlPct+=pnl;if(pnl>0)wins++;}
    ...
    // Stop-Anzeige: Stop/Ziel kompakt
    const stopStr=p.stop?\`\${p.stop}€\`:'—';
    ...
    return \`<tr>
      ...
      <td>\${pctHtml(chg)}</td>
      <td>\${pctHtml(pnl)}</td>
      <td class="muted" style="font-size:12px">\${stopStr} / \${targetStr}</td>
    </tr>\`;
  });

  const avgPnl=count?totalPnlPct/count:0;
  const wr=count?(wins/count*100):0;
  // Schätzung Aktueller Wert: START * (1 + avgPnl/100 * 0.9) + CASH
  const curVal=Math.round(posValue*(1+avgPnl/100)+CASH);
  const totalPnlEur=curVal-START;
  ...
  document.getElementById('paper-stats').innerHTML=\`
    ...
    <div class="stat"><div class="stat-label">Akt. Wert</div><div class="stat-value \${curVal>=START?'green':'red'}">\${curVal}€</div><div class="stat-sub \${totalPnlEur>=0?'green':'red'}">\${totalPnlEur>=0?'+':''}\${totalPnlEur}€</div></div>
    <div class="stat"><div class="stat-label">Ø P&amp;L</div><div class="stat-value \${avgPnl>=0?'green':'red'}">\${avgPnl>=0?'+':''}\${avgPnl.toFixed(1)}%</div></div>
    <div class="stat"><div class="stat-label">Win-Rate</div><div class="stat-value">\${wr.toFixed(0)}%</div><div class="stat-sub muted">\${wins}/\${count} Trades</div></div>
    ...
    <div class="strat-card">
      <div class="strat-header" onclick="toggleStrat(\${i})">
        ...
        <div class="strat-title">
          <div class="strat-name">\${s.status} \${s.name}</div>
      ...
      <div class="strat-body" id="strat-body-\${i}">
        \${s.desc.replace(/\\n/g,'<br>')}
        <div class="strat-tickers">
          \${s.tickers.map(t=>\`<span class="badge" style="background:\${s.color}22;color:\${s.color}">\${t}</span>\`).join('')}
        </div>
      </div>
    </div>\`).join('');
}
...
function toggleStrat(i) {
  const body=document.getElementById('strat-body-'+i);
...
async function savePos(ticker) {
  ...
  try {
    ...
    if(d.status==='ok'){
      el.textContent='✅ gespeichert';el.style.color='var(--green)';
      setTimeout(()=>{el.textContent='';loadAll();},1500);
...
function setAction(a) {
  tradeAction=a;
  const buy=document.getElementById('btn-buy'),sell=document.getElementById('btn-sell');
  buy.className='btn btn-sm '+(a==='BUY'?'btn-primary':'btn-muted');
  sell.className='btn btn-sm '+(a==='SELL'?'btn-danger':'btn-muted');
}
